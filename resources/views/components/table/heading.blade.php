...
<th
    {{ $attributes->merge(['class' => 'px-6 py-3 whitespace-nowrap'])->only('class') }}
>
    @unless ($sortable)
        <span class="flex items-center space-x-1 {{ $textAlign }} text-xs leading-4 font-bold uppercase text-gray-600 dark:text-gray-200">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 {{ $textAlign }} text-xs leading-4 font-bold uppercase text-gray-600 dark:text-gray-200 group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
